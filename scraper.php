...
$term_url = "https://online.kingston.vic.gov.au/ePathway/Production/Web/GeneralEnquiry/EnquiryLists.aspx";
...
foreach ($dataset as $record) {
    ...
    $date_received = trim(html_entity_decode($record->find('td',5)->plaintext));
    $date_received = explode('/', $date_received);
    $date_received = "$date_received[2]-$date_received[1]-$date_received[0]";
    
    $address = trim(html_entity_decode($record->find('td', 1)->plaintext)) . ', ' .
               trim(html_entity_decode($record->find('td', 2)->plaintext)) . ', ' . 'VIC';
